...
- 株式会社ぐるなび ぐるなびウェブサービスAPIや社内情報を使った新サービスアイディアソン
...
- 言語：Python
- 背景：プライムデーで買ったdash button、熱盛事故。これは組み合わせるしかない。このビッグウェーブ、乗るしかない。と２日間で完成させたもの。
...
- 開発年：2017年6月-
- 言語：Python
- 背景:機械学習で絵師さんのツイットだけが見れるリスト等から俺好みの絵だけを表示させるクライアントが欲しかったので作る。(また、Pythonistaのたしなみとして機械学習に手を付けておくべきかなとも思っていたのも背景に少しあり)
...
-- というわけでこちらのスライド、パラメータをあげないと最初のスライド２枚が表示されない仕様になっているはずです。 →現在絶賛開発中&API仕様変更中の為、どうやっても表示される可能性
- Github:https://github.com/mecaota/mecaota.github.io
...
- 開発年：2017年9月1日-
- 言語：Python(深層学習のライブラリはKeras, バックエンドはtensorflow)
...
- 開発年:2017年8月1日-
- 言語:Arduino(C++)
- 背景:冒頭で話した、1年次に作ったプログラムとは別に、車のOBD2端子→sakura.io→AWS IoT gateway→未定の流れで、車載装置で下宿のカードキーを乗降時にそれぞれ読み取らせると自動でガソリン精算してくれ、終いにはメールまで自動送信してくれるシステムの開発をしたい。

- →下宿車のプロトコルが旧式軽自動車特有のガラパゴスプロトコルにより、仕組み的に不可能な可能性が出てきており、GPSシールドを使ったGPS方式に切り替えるのを検討中。
- Github:https://github.com/mecaota/OBD2_sending

---

## 今やりたいこと

- golangでバックエンドシステム作ってみたい
- GCPではやく機械学習回してみたい
...
まだどのレイヤーの開発に携わりたいか、どの言語のプロになりたいのかなど定まっていない。また、FinTechに興味こそあるが、どの業界でも開発に携わって自分の腕を磨けるなら業界に拘る必要がないと今は思っている。

フルスタックエンジニアとして働けるようになりたい。

---

## 会社に望むこと・希望業界など

- できるならば、いろんな分野や業界の開発に携わりたい
- できるだけ最新の技術を使っている・使おうとしている会社に行きたい
- 自分の技術力を磨ける・上げられる会社
- 技術選定をしっかり行っている会社に行きたい
- ↑に関連して、いろんな技術・レイヤー・言語に触れられる機会があるところに行きたい

---

冬のインターン、ありましたら、ぜひ参加したいです。

もしマッチングしそうな会社さんとはぜひ選考応募させてください！
